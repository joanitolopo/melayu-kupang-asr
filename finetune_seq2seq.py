...
def load_model_and_tokenizer(model_args):
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(model_args.model_name_or_path, config=quant_config)
...
def prepare_datasets(data_args, tokenizer):
    ...
    def tokenize_function(data_point):
        ...
        source_ids = tokenizer(user_prompt, truncation=True, max_length=data_args.source_max_length)["input_ids"]
        target_ids = tokenizer(text_target=target_text, truncation=True, max_length=data_args.model_max_length - data_args.source_max_length)["input_ids"]
    ...
    train_dataset = train_val_split["train"].map(tokenize_function, remove_columns=train_val_split["test"].column_names, desc="Tokenizing train dataset")
    val_dataset = train_val_split["test"].map(tokenize_function, remove_columns=train_val_split["test"].column_names, desc="Tokenizing validation dataset")
